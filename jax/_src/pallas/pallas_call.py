...
def _pallas_call_batching_rule(
  ...
  if lengths_aval:
    ...
    for eqn in jaxpr.eqns:
      ...
      rule(var_to_raggedness, eqn.invars, eqn.outvars)

    for pos, (invar, _) in enumerate(zip(jaxpr.invars, ragged_axis_values)):
  ...
  def wrapped(*args):
    ...
    for i_idx, o_idx in input_output_aliases.items():
      ...
      if isinstance(in_aval, jax_core.DShapedArray):
        ...
        in_aval = jax_core.ShapedArray(tuple(new_shape), in_aval.dtype, in_aval.weak_type)
