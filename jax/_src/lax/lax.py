...
def _dot_general_ragged_prop_rule(var_to_raggedness_table, invars, outvars):
  assert len(invars) == 2
  ...
  var_to_raggedness_table[outvars[0]] = dense_jumble_raggedness

  return dense_jumble_raggedness
...
def _broadcast_in_dim_ragged_prop_rule(
    var_to_raggedness_table, invars, outvars
):
  assert len(invars) == 1
  assert not isinstance(invars[0], core.Var)
